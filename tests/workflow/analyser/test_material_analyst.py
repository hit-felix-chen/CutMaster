...
def test_shot_sampling_repeats_last_decodable_frame(monkeypatch, tmp_path) -> None:
    ...
    capture = FakeCapture()
    monkeypatch.setattr(
        "cutmaster.workflow.analyser.material_analyst.cv2.VideoCapture",
...
def test_full_video_shot_detection_builds_complete_boundary_partition(monkeypatch) -> None:
    monkeypatch.setattr(
        "cutmaster.workflow.analyser.material_analyst.detect_source_cuts",
...
def test_segment_annotation_uses_one_parallel_vlm_call_per_segment(
    ...
    monkeypatch.setattr(
        "cutmaster.workflow.analyser.material_analyst._sample_shot_frames",
        lambda *_args: (
...
def test_segment_annotation_packs_frames_when_provider_image_limit_is_exceeded(
    ...
    monkeypatch.setattr(
        "cutmaster.workflow.analyser.material_analyst._shot_frame_contact_sheet",
        pack,
    )
...
def test_analyser_reuses_shot_checkpoint_after_later_stage_failure(
    ...
    monkeypatch.setattr(
        "cutmaster.workflow.analyser.material_analyst.probe_media",
            lambda _path: {
                "duration": 2.0,
                "fps": 24.0,
                "width": 1920,
    ...
    monkeypatch.setattr("cutmaster.workflow.analyser.material_analyst._detect_full_video_shots", detect)
    monkeypatch.setattr(
        "cutmaster.workflow.analyser.material_analyst.prepare_subtitles",
    ...
    assert (tmp_path / "analysis" / "shots.json").is_file()


def test_complete_analysis_cache_requires_exact_signature(tmp_path) -> None:
    memory = tmp_path / "analysis"
    segment_clip = memory / "segments" / "segment_0001.mp4"
    ...
    summary = {
        ...
        "synopsis": "A complete test story.",
        "chronological_story_beats": [
            {
                "source_segment_ids": ["segment_0001"],
                "summary": "Opening",
                "characters": [],
                "narrative_significance": "Introduces the story",
            }
        ],
        "character_arcs": [],
        "themes": ["test"],
