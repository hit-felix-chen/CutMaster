...
from cutmaster.renderer.ffmpeg import RenderError
from cutmaster.renderer.renderer import render_montage
...
def test_auto_encoder_falls_back_to_libx264(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "cutmaster.renderer.renderer.check_media_tools",
        lambda: None,
    )
    monkeypatch.setattr(
        "cutmaster.renderer.renderer.probe_media",
        lambda _path: {"duration": 10.0},
    )
    monkeypatch.setattr(
        "cutmaster.renderer.renderer.select_encoder",
        lambda _requested: "h264_videotoolbox",
    )
    monkeypatch.setattr(
        "cutmaster.renderer.renderer.render_clip",
        fake_render_clip,
    )
    monkeypatch.setattr(
        "cutmaster.renderer.renderer.concatenate_clips",
        lambda _clips, output: output.write_bytes(b"montage"),
    )
    monkeypatch.setattr(
        "cutmaster.renderer.renderer.mix_bgm",
