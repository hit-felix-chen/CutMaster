...
def test_detect_beats_uses_librosa_tracker_and_repeats_bgm(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cutmaster.music.beats.librosa.load", lambda *_args, **_kwargs: (samples, 22050))
    monkeypatch.setattr(
        "cutmaster.music.beats.librosa.onset.onset_strength",
    ...
    monkeypatch.setattr("cutmaster.music.beats.librosa.beat.beat_track", fake_track)
    monkeypatch.setattr(
        "cutmaster.music.beats.librosa.frames_to_time",
        lambda *_args, **_kwargs: np.array([0.5, 1.5]),
    )
    monkeypatch.setattr("cutmaster.music.beats.librosa.get_duration", lambda **_kwargs: 2.0)
...
def test_detect_beats_rejects_audio_without_tracked_beats(monkeypatch) -> None:
    monkeypatch.setattr(
        "cutmaster.music.beats.librosa.load",
    ...
    monkeypatch.setattr(
        "cutmaster.music.beats.librosa.beat.beat_track",
