...
class SettingsService:
    ...
    def reload(self) -> SettingsView:
        """Reload the exact selected configuration file from disk."""
    ...
    def save(self, command: SaveSettingsCommand) -> SavedSettingsView:
        updates = _copy_json_mapping(command.values)
        config_path = self._effective_configuration.sources.base_path
        lock_path = _settings_lock_path(config_path)
        ...
        with lock_path.open("a+b") as lock:
            ...
            previous = _require_configuration_bytes(config_path)
            content = _updated_configuration(previous, updates)
            wrote_candidate = False

            def apply() -> Mapping[str, Any]:
                nonlocal wrote_candidate
                _validate_candidate_configuration(config_path, content)
                _atomic_write(config_path, content)
                wrote_candidate = True
                loaded = load_effective_configuration(config_path)
                return {
            ...
            try:
                ...
                if wrote_candidate:
                    _restore_file(config_path, previous)
                raise
            finally:
                fcntl.flock(lock.fileno(), fcntl.LOCK_UN)

        if receipt_exists:
            expected_values = result.value.get("values")
            if expected_values != updates:
                raise RuntimeError(
                    "Settings receipt does not match its canonical values"
                )
            current = _read_existing_regular_file(config_path)
            if current is None or current.decode("utf-8") != content:
                raise RuntimeError(
                    "Configuration no longer matches the completed command receipt"
                )
    ...
    def save_providers(
        ...
        """Atomically save public providers to config.toml and secrets to .env."""
        ...
        previous_configuration = _require_configuration_bytes(config_path)
        provider_updates = _provider_configuration_updates(providers)
        configuration_content = _updated_configuration(
            previous_configuration,
            provider_updates,
        )
        _validate_candidate_configuration(config_path, configuration_content)
        ...
        with lock_path.open("a+b") as lock:
            ...
            def apply() -> Mapping[str, Any]:
                nonlocal wrote_dotenv, wrote_configuration
                # Re-read after acquiring the lock so concurrent saves cannot be lost.
                locked_configuration = _read_existing_regular_file(config_path)
                locked_dotenv = _read_existing_regular_file(dotenv_path)
                if (
                    locked_configuration != previous_configuration
                ...
                if dotenv_changed:
                    _atomic_write(dotenv_path, dotenv_content, mode=0o600)
                    wrote_dotenv = True
                _atomic_write(config_path, configuration_content)
            ...
            try:
                ...
                if wrote_configuration:
                    _restore_file(config_path, previous_configuration)
                if wrote_dotenv:
        ...
        if receipt_exists:
            current_configuration = _read_existing_regular_file(config_path)
            if (
                current_configuration is None
                or current_configuration.decode("utf-8") != configuration_content
            ):
                raise RuntimeError(
                    "Provider configuration no longer matches the completed "
                    "command receipt"
                )
...
def _provider_configuration_updates(
    providers: Mapping[str, Mapping[str, Any]],
) -> dict[str, Any]:
    return {
        "llm": _copy_json_mapping(providers["llm"]),
        "vlm": _copy_json_mapping(providers["vlm"]),
        "analyser": {"asr": _copy_json_mapping(providers["asr"])},
    }
...
def _copy_json_mapping(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("Settings values must be JSON-compatible") from exc
    if not isinstance(result, dict):
        raise TypeError("Settings values must be an object")
    return result


def _updated_configuration(previous: bytes, updates: Mapping[str, Any]) -> str:
    document = tomlkit.parse(previous.decode("utf-8"))

    def merge(target: Any, values: Mapping[str, Any]) -> None:
...
def _validate_candidate_configuration(config_path: Path, content: str) -> None:
    descriptor, temporary_name = tempfile.mkstemp(
        dir=config_path.parent,
        prefix=f".{config_path.stem}.settings-validation.",
        suffix=".toml",
    )
    ...
    try:
        ...
        load_effective_configuration(temporary)
    finally:
        temporary.unlink(missing_ok=True)


def _settings_lock_path(config_path: Path) -> Path:
    return config_path.parent / ".cutmaster-control" / f"{config_path.name}.lock"


def _require_configuration_bytes(path: Path) -> bytes:
    value = _read_existing_regular_file(path)
    if value is None:
...
def _read_existing_regular_file(path: Path) -> bytes | None:
    ...
    if not stat.S_ISREG(metadata.st_mode):
        raise ValueError(f"Settings file must be a regular file: {path}")
