...
from cutmaster.workflow.renderer.dialogue_audio import prepare_dialogue_audio
from cutmaster.workflow.renderer.ffmpeg import (
    RenderError,
    encoder_args,
    run_media_command,
    select_encoder,
...
from cutmaster.infrastructure.media.ffprobe import (
    check_media_tools,
    media_duration,
    media_frame_count,
    probe_media,
...
class Renderer:
    ...
    def render(
        ...
        *,
        ...
        video_path = request.bindings.video.source_path.resolve()
        audio_path = request.bindings.music.source_path.resolve()
        for source, label in (
            (video_path, "Video Material source"),
            (audio_path, "Music Material source"),
        ):
            if not source.is_file() or source.is_symlink():
                raise FileNotFoundError(f"{label} is unavailable: {source}")
        output_dir = request.output_target.root.resolve()
        if output_dir.exists() and (not output_dir.is_dir() or output_dir.is_symlink()):
            raise ValueError(f"Render output root is not a regular directory: {output_dir}")
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / request.output_target.output_filename
        if output_path.exists() and not overwrite:
            raise FileExistsError(
                f"Rendered output already exists: {output_path}"
            )
        ...
        if request.options.audio_mode == "dialogue":
        ...
        if montage_reused:
            mix_bgm(
                ...
                plan.duration_sec,
                source_video=video_path,
                script=script if request.options.audio_mode == "dialogue" else None,
        ...
        result = RenderResult(
            ...
            audio_mode=request.options.audio_mode,
            output_video=output_path.resolve(),
            montage_video=montage_path.resolve(),
